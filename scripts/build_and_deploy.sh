...
build_and_publish_pg_extension() {
  ...
  release_url="https://github.com/gexops/pg-extensions/releases/tag/$PG_EXTENSION_NAME-v$SANITIZED_PG_EXTENSION_VERSION-pg$PG_MAJOR_VERSION-$ARCH-ubuntu$UBUNTU_VERSION"
  if curl --output /dev/null --silent --head --fail "$release_url"; then
    ...
    release_response=$(curl -s -X POST https://api.github.com/repos/gexops/pg-extensions/releases \
